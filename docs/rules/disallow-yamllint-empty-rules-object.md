...
Disallow empty rules object keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/disallow-yamllint-empty-rules-object`.
...
export default [
 ...
 {
  rules: {
   "yamllint/disallow-yamllint-empty-rules-object": "error",
  },
 },
];
```
