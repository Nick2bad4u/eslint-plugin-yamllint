...
Disallow conflicting ignore keys keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/disallow-yamllint-conflicting-ignore-keys`.
...
The rule reports yamllint configuration patterns that do not match the bridge's supported policy surface.
...
export default [
 ...
 {
  rules: {
   "yamllint/disallow-yamllint-conflicting-ignore-keys": "error",
  },
 },
];
```
