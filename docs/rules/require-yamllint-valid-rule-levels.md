...
Require valid rule levels keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/require-yamllint-valid-rule-levels`.
...
The rule reports invalid yamllint rule levels so config files only use supported severity values.
...
export default [
 ...
 {
  rules: {
   "yamllint/require-yamllint-valid-rule-levels": "error",
  },
 },
];
```
