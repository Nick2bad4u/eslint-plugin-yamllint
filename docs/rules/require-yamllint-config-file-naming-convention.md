...
Require config file naming convention keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/require-yamllint-config-file-naming-convention`.
...
The rule reports .yamllint alternatives that do not match the conventional file names supported by this bridge.
...
export default [{
  rules: {
    "yamllint/require-yamllint-config-file-naming-convention": "error"
  }
}];

// config file present: yamllint.config.yaml
```

## ✅ Correct

```yaml
// config file present: .yamllint
```
