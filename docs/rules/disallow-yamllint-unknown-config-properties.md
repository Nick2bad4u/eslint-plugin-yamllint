...
Disallow unknown config properties keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/disallow-yamllint-unknown-config-properties`.
...
The rule reports unknown or misspelled properties in yamllint configuration objects before they silently drift from the upstream schema.
...
export default [
 ...
 {
  rules: {
   "yamllint/disallow-yamllint-unknown-config-properties": "error",
  },
 },
];
```
