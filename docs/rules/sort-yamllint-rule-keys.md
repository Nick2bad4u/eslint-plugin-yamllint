...
Sort rule keys keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/sort-yamllint-rule-keys`.
...
The rule reports arrays or object keys that are not sorted, keeping yamllint config review diffs stable.
...
export default [
 ...
 {
  rules: {
   "yamllint/sort-yamllint-rule-keys": "error",
  },
 },
];
```
