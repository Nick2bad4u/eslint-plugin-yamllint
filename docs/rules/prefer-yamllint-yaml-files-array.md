...
Prefer yaml files array keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/prefer-yamllint-yaml-files-array`.
...
export default [
 ...
 {
  rules: {
   "yamllint/prefer-yamllint-yaml-files-array": "error",
  },
 },
];
```
