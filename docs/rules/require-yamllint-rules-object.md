...
Require rules object keeps yamllint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `yamllint/require-yamllint-rules-object`.
...
The rule reports missing required yamllint config structure for require rules object.
...
export default [
 ...
 {
  rules: {
   "yamllint/require-yamllint-rules-object": "error",
  },
 },
];
```
